...
class EventHandler {
    ...
    public async run(): Promise<void> {
        return new Promise<void>((resolve) => {
            readdirSync(join(__dirname, "../event")).filter(file => file.endsWith(".ts")).forEach(file => {
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const { event }: { event: IEvent } = require(join(__dirname, "../event", file));
                ...
                if(event) {
                    ...
                    if(once) {
                        this.bot.once(name, (...args: unknown[]) => {
                            event.handler(this.bot, ...args).catch(error => {
                                this.bot.getLogger().fatal(new Error(`
                    ...
                    this.bot.getLogger().info(`Registered ${once ? "one time" : "ongoing"} event ${name}`);
                }
            });
