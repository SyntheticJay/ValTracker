...
export const command: ICommand = {
	...
	handler: async(bot: ValorantTracker, message: Message, args: any[]): Promise<void> => {
		...
		bot.getDatabaseHandler().setPrefix(message.server, prefix).catch(error => {
			bot.getLogger().error(new Error(`There was an error setting prefix for server ${message.server.id}: ${error}`));
			message.reply(`An error occurred while setting the prefix. Please try again later.`);
		});

		message.reply(`The prefix for **${message.server.raw}** has been set to \`${prefix}\`.`);
	}
}