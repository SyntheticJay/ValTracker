...
export const event: IEvent = {
    ...
    handler: async(bot: ValorantTracker, message: Message): Promise<void> => {
		const prefix = await bot.getDatabaseHandler().getPrefix(message.server).catch(error => {
			bot.getLogger().error(new Error(`Failed to get prefix for ${message.server.id}: ${error}`));
		});

		bot.getCommandHandler().run(message, prefix || bot.getConfig().defaultPrefix).catch(async error => {
